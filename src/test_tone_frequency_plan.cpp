...
TestToneFrequencyPlanResult plan_explicit_test_tone_frequency(const ParsedTestToneRequest&r,double offset){
 ...
 for(const auto&e:l.canonical_wspr_band_catalog())if(e.band==r.band){if(offset_hz>std::numeric_limits<std::uint64_t>::max()-e.dial_frequency_hz)return fail("audio offset overflows RF frequency");return {TestToneFrequencyPlan{r.source,e.band,e.dial_frequency_hz,offset_hz,e.dial_frequency_hz+offset_hz}, {}};}
