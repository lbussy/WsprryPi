...
int main() {
    ...
    assert(service.find("/usr/bin/systemd-run") != std::string::npos);
    assert(service.find("\"--no-block\"") != std::string::npos);
    assert(service.find("\"--collect\"") != std::string::npos);
    assert(service.find("RuntimeMaxSec=90s") != std::string::npos);
