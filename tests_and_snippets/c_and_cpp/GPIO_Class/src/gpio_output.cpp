 ...
 * @details Calls stop_gpio_pin() and releases resources.
 */
...
bool GPIOOutput::toggle_gpio(bool state)
{
    if (!enabled_ || !line_)
    {
        std::cerr << "GPIO pin not enabled." << std::endl;
        return false;
    }
    try
    {
    ...
    {
        std::cerr << "Error toggling GPIO pin " << pin_ << ": " << e.what()
                  << std::endl;
        return false;
    }
    return true;
